...
def get_db_connection():
    return pymssql.connect(server='votaciones.mssql.somee.com', user='krzDev506_SQLLogin_1',
                           password='ojj5uucf8i', database='votaciones')
...
def mostrar_imagen(idP):
    ...
    try:
        ...
        if imagen and imagen[0]:
            ...
            return send_file(io.BytesIO(imagen[0]), mimetype='image/jpeg')
        else:
...
def get_votos():
    ...
    try:
        ...
        return jsonify([row for row in votos])  # No need for dictionary conversion
    except Exception as e:
